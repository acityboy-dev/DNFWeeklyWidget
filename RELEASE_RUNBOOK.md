...
또는 루트의 `publish-release.bat 0.1.4`를 사용할 수 있다. 버전 인자를 생략하면 프로젝트의 `<Version>`을 읽는다.
...
메인 앱과 업데이터는 모두 `win-x64`, self-contained, single-file 옵션으로 publish한다. WPF 네이티브 DLL이 함께 생성될 수 있으며 ZIP에 모두 포함해야 한다.
...
배포 저장소에 `v0.1.4` GitHub Release를 만들고 다음 ZIP을 Release Asset으로 업로드한다.

```text
DNFWeeklyWidget-0.1.4-win-x64.zip
```

다운로드 URL 형식은 다음과 같다.

```text
https://github.com/acityboy-dev/DNFWeeklyWidget.Release/releases/download/v0.1.4/DNFWeeklyWidget-0.1.4-win-x64.zip
```
...
{
  "version": "0.1.4",
  "packageUrl": "https://github.com/acityboy-dev/DNFWeeklyWidget.Release/releases/download/v0.1.4/DNFWeeklyWidget-0.1.4-win-x64.zip",
  "sha256": "ZIP_SHA256",
...
```powershell
git add update.json
git commit -m "0.1.4 업데이트 매니페스트 배포"
git push origin main
```

`v0.1.4` 태그는 배포 저장소의 최신 매니페스트 커밋을 가리키게 맞춘다. 태그를 이동해야 한다면 원격 태그도 명시적으로 갱신한다.
